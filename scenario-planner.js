...
(function(){
  ...
  /* ---- charts ---- */
  const GOLD='#A9842A', GOLD_BRIGHT='#E4C56B', GREY='#9A958C', GREEN='#2E8B57', RED='#C0563F';
  const baseOpts=(yFmt)=>({
    responsive:true,maintainAspectRatio:false,animation:reduce?false:{duration:500},
    interaction:{mode:'index',intersect:false},
    plugins:{legend:{display:false},tooltip:{
      backgroundColor:'#16161A',borderColor:GOLD,borderWidth:1,titleColor:'#F3F1EA',bodyColor:'#D8D4CB',
      padding:11,cornerRadius:8,titleFont:{family:'Space Mono'},bodyFont:{family:'DM Sans'},
      callbacks:{label:c=>' '+c.dataset.label+': '+(yFmt?yFmt(c.parsed.y):c.parsed.y)}
    }},
    scales:{
      x:{grid:{color:'rgba(0,0,0,.05)'},ticks:{color:'#6E6A62',font:{family:'Space Mono',size:10}}},
      y:{grid:{color:'rgba(0,0,0,.05)'},ticks:{color:'#6E6A62',font:{family:'Space Mono',size:10},callback:v=>yFmt?yFmt(v):v}}
    }
  });
  ...
  function drawCharts(exp,best,worst){
    ...
    const revData={labels,datasets:[
      {label:'Best',data:best.revenue,borderColor:GOLD_BRIGHT,backgroundColor:'rgba(228,197,107,.12)',borderWidth:1.5,borderDash:[5,4],pointRadius:0,tension:.3,fill:'+1'},
      {label:'Expected',data:exp.revenue,borderColor:GOLD,backgroundColor:'rgba(169,132,42,.14)',borderWidth:3,pointRadius:0,tension:.3,fill:'+1',pointHoverRadius:5},
    ...
    ebOpts.scales.y1={position:'right',grid:{drawOnChartArea:false},ticks:{color:'#6E6A62',font:{family:'Space Mono',size:10},callback:v=>v.toFixed(0)+'%'}};
    const ebData={labels,datasets:[
      {type:'bar',label:'EBITDA',data:exp.ebitda,backgroundColor:'rgba(169,132,42,.78)',borderRadius:4,yAxisID:'y',order:2},
    ...
    const custData={labels,datasets:[
      {label:'Customers',data:exp.customers.map(Math.round),borderColor:GOLD,backgroundColor:'rgba(169,132,42,.12)',borderWidth:3,pointRadius:3,pointBackgroundColor:GOLD,tension:.3,fill:true}
    ]};
